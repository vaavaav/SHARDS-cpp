...
class SplayTree
{
  struct Node
  {
    ...
    Node(KeyType key, uint64_t size) : key(key), size(size) {}
    Node(KeyType key, uint64_t size, Node *left, Node *right)
        : key(key), size(size), children{left, right} {}
  };

  static inline uint64_t node_size(Node *node)
  {
...
public:
  ...
  uint64_t greater_or_equal_to(KeyType key) const;
  ~SplayTree()
  {

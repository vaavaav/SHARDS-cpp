...
ShardsFixedSize::ShardsFixedSize(uint64_t T0, uint64_t P, uint32_t SMax, uint32_t bucketSize)
    : kP{P}, kSMax{SMax}, kBucketSize{bucketSize}, m_R{static_cast<double>(T0) / P}, m_T{T0}
{
    assert(bucketSize > 0 && SMax > 0);
}

ShardsFixedSize::ShardsFixedSize(double R0, uint32_t SMax, uint32_t bucketSize)
...
uint32_t ShardsFixedSize::getDistance(std::string const &key)
{
    uint32_t distance = 0;
...
std::map<uint32_t, double> ShardsFixedSize::mrc()
{
